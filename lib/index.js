...
function shp(base, whiteList = [], proj4Str = null) {
  if (typeof base === 'string' && cache.has(base)) {
    return Promise.resolve(cache.get(base));
  }
  return shp.getShapefile(base, whiteList, proj4Str).then(function (resp) {
...
shp.parseZip = async function (buffer, whiteList, proj4Str = null) {
  let key;
...
async function getZip(base, whiteList, proj4Str = null) {
  const a = await binaryAjax(base);
  return shp.parseZip(a, whiteList, proj4Str);
}
...
shp.getShapefile = async function (base, whiteList, proj4Str = null) {
  if (typeof base !== 'string') {
    return shp.parseZip(base, [], proj4Str);
  }
